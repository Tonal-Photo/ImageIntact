...
struct ContentView: View {
  ...
  func showDebugLog() {
    ...
    let tempLogPath = tempDir.appendingPathComponent(
      "ImageIntact_Session_\(sessionID.isEmpty ? "Latest" : sessionID).log")
  ...
  func exportDebugLog() {
    ...
    savePanel.nameFieldStringValue =
      "ImageIntact_Debug_\(dateFormatter.string(from: Date()))\(suffix).txt"
  ...
  func getLargeBackupMessage(info: BackupManager.LargeBackupInfo) -> String {
    ...
    if info.destinationCount > 1 {
      let totalSize = formatter.string(
        fromByteCount: info.totalBytes * Int64(info.destinationCount))
      return """
