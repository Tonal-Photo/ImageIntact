    ...
    Button(action: action) {
      ...
      .background(
        RoundedRectangle(cornerRadius: 6)
          .fill(
            isSelected ? Color.accentColor : (isHovered ? Color.gray.opacity(0.1) : Color.clear))
      )
    }
    .buttonStyle(.plain)
...
struct PresetDetailView: View {
      ...
      VStack(alignment: .leading, spacing: 20) {
        ...
        VStack(alignment: .leading, spacing: 16) {
          ...
          DetailSection(title: "Destinations", icon: "externaldrive.badge.checkmark") {
            Text(
              "\(preset.destinationCount) \(preset.destinationCount == 1 ? "destination" : "destinations")"
            )
            .fontWeight(.medium)
          }

          if preset.excludeCacheFiles || preset.skipHiddenFiles || preset.preventSleep
            || preset.showNotification
          {
  ...
  private func getDetailedDescription() -> String {
    ...
    case "Daily Workflow":
      return
        "Perfect for daily photography work. Backs up only image files incrementally with balanced speed and verification."
    case "Travel Backup":
      return
        "Optimized for on-location backup. Focuses on RAW files with fast processing and automatic start on drive connect."
    case "Client Delivery":
      return
        "Ensures perfect copies for client delivery. Creates exact mirrors of processed images with full verification."
    case "Archive Master":
      return
        "Comprehensive archival solution. Backs up everything with maximum verification to multiple destinations for long-term preservation."
    case "Video Project":
      return
        "Handles large video files efficiently. Fast incremental backup without verification bottlenecks."
    case "Quick Mirror":
