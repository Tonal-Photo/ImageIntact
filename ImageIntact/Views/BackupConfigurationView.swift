...
struct BackupConfigurationView: View {
  ...
  var body: some View {
    HStack(spacing: 8) {
      ...
      Menu {
        ...
        let isDuplicatePreset = presetManager.currentConfigurationMatchesExistingPreset(
    ...
    .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("ShowCreatePreset"))) {
      _ in
      showingCreatePreset = true
    }
  }
  ...
  private func getPresetDescription(_ preset: BackupPreset) -> String {
    ...
    case "Daily Workflow":
      return
        "Incremental backup of photos only, balanced performance. Ideal for regular photographer workflow with 2 destinations."
    case "Travel Backup":
      return
        "Fast RAW-only backup, starts on drive connect. Perfect for on-location backup to a portable drive."
    case "Client Delivery":
      return
        "Mirror copy of JPEG/TIFF with full verification. Ensures exact copies for client delivery."
    case "Archive Master":
      return
        "Complete archive of all files with full verification to 3 destinations. For long-term preservation."
    case "Video Project":
      return
        "Fast incremental backup of video files. Optimized for large video projects to 2 destinations."
    case "Quick Mirror":
