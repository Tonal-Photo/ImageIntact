...
struct OrganizationSection: View {
    ...
    /// Recent folder names from preferences
    private var recentFolderNames: [String] {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 8) {
                    ...
                    // Recent folder names dropdown
                    if !recentFolderNames.isEmpty {
                        Menu {
                            ForEach(recentFolderNames, id: \.self) { name in
                                Button(name) {
                                    backupManager.organizationName = name
                                }
                            }
                        } label: {
                    ...
                    .popover(isPresented: $showInfoPopover, arrowEdge: .trailing) {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            if !backupManager.organizationName.isEmpty
                                && backupManager.destinationItems.count > 0
                            {
                                Divider()
                ...
                HStack(spacing: 4) {
                    ...
                    {
                        Text(
                            "\(destURL.lastPathComponent)/\(backupManager.organizationName.isEmpty ? "folder-name" : backupManager.organizationName)/"
                        )
                        ...
                        Text(
                            "[destination]/\(backupManager.organizationName.isEmpty ? "folder-name" : backupManager.organizationName)/"
                        )
                        .font(.system(size: 11, design: .monospaced))
                        .foregroundColor(.accentColor)
                        .fontWeight(.medium)
