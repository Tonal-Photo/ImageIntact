                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(spacing: 12) {
                            HStack(alignment: .center, spacing: 12) {
                                ...
                                Text(
                                    preferences.defaultSourcePath.isEmpty
                                        ? "Not set"
                                        : URL(fileURLWithPath: preferences.defaultSourcePath).lastPathComponent
                                )
                                .font(.system(size: 13))
                            ...
                            HStack(alignment: .center, spacing: 12) {
                                ...
                                Text(
                                    preferences.defaultDestinationPath.isEmpty
                                        ? "Not set"
                                        : URL(fileURLWithPath: preferences.defaultDestinationPath).lastPathComponent
                                )
                                .font(.system(size: 13))
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Toggle("Restore last session paths on launch", isOn: $preferences.restoreLastSession)
                                .font(.system(size: 13))
                                .help(
                                    "Automatically reload the source and destination folders from your last session")
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            Toggle("Exclude cache files by default", isOn: $preferences.excludeCacheFiles)
                                .font(.system(size: 13))
                                .help(
                                    "Skip Adobe cache files, Lightroom previews, .DS_Store, thumbnails, and other temporary files"
                                )
                ...
                VStack(alignment: .leading, spacing: 20) {
                        ...
                        GroupBox {
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                HStack(spacing: 12) {
                                    ...
                                    if let info = systemInfo {
                                        if info.performanceCores > 0 {
                                            Text(
                                                "\(info.cpuCores) total (\(info.performanceCores) performance, \(info.efficiencyCores) efficiency)"
                                            )
                                            .font(.system(size: 13))
                                        } else {
                                            Text("\(info.cpuCores)")
                                ...
                                HStack(spacing: 12) {
                                    ...
                                    if let info = systemInfo {
                                        Text(
                                            ByteCountFormatter.string(fromByteCount: info.totalRAM, countStyle: .binary)
                                        )
                                        .font(.system(size: 13))
                                    }
                                    Spacer()
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            Toggle("Enable Vision Framework features", isOn: $preferences.enableVisionFramework)
                                ...
                                .onChange(of: preferences.enableVisionFramework) { _, newValue in
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                Toggle(
                                    "Use stream-based copy for network volumes",
                                    isOn: $preferences.useStreamCopyForNetwork
                                )
                                .font(.system(size: 13))
                                .help(
                                    "Enable cancellable, throttleable copying for network drives (recommended for SMB)"
                                )
                                ...
                                .popover(isPresented: $showStreamCopyInfo, arrowEdge: .trailing) {
                                    VStack(alignment: .leading, spacing: 12) {
                                        ...
                                        Text(
                                            "When enabled, ImageIntact uses a chunk-based copying method for network drives that provides:"
                                        )
                                        .font(.system(size: 12))
                            ...
                            HStack(spacing: 12) {
                                ...
                                TextField(
                                    "", value: $preferences.networkCopySpeedLimit,
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Toggle(
                                ...
                                isOn: $preferences.showNotificationOnComplete
                            )
                            .font(.system(size: 13))
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                Toggle("Verbose Logging", isOn: $debugSettings.verboseLogging)
                                    .font(.system(size: 13))
                                    .help("Show all debug messages. Resets when app quits.")

                                Text("Enables detailed diagnostic output for troubleshooting. This setting is not saved and will reset when ImageIntact quits.")
                                    .font(.system(size: 11))
                                    .foregroundColor(.secondary)
                                    .fixedSize(horizontal: false, vertical: true)
                            }
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text(
                            "ImageIntact maintains a local, private record of all your backups on this Mac to help you locate files in the future. This data never leaves your computer."
                        )
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack(alignment: .top, spacing: 8) {
                                Toggle(
                                    "Anonymize paths when exporting logs", isOn: $preferences.anonymizePathsInExport
                                )
                                .font(.system(size: 13))
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            Toggle(
                                "Enable smart duplicate detection", isOn: $preferences.enableSmartDuplicateDetection
                            )
                            .font(.system(size: 13))
                            .help(
                                "Detect and skip files that are already backed up at destinations using checksums")

                            Toggle(
                                "Show technical details during backup", isOn: $preferences.showTechnicalDetails
                            )
                            .font(.system(size: 13))
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Toggle("Show summary before starting backup", isOn: $preferences.showPreflightSummary)
                                .font(.system(size: 13))
                                .help(
                                    "Display a summary of what will be backed up before starting. You can also toggle this directly from the summary dialog."
                                )
