...
struct HelpWindowView: View {
  ...
  enum HelpSectionID: String, CaseIterable {
    ...
    var searchTerms: [String] {
      ...
      case .fileTypes:
        return ["raw", "jpeg", "video", "sidecar", "format", "capture one", "lightroom"]
      ...
      case .faq:
        return ["question", "why", "symlink", "symbolic", "link", "alias", "skip", "not copying"]
      ...
      GroupBox {
        VStack(alignment: .leading, spacing: 12) {
          Label("Stream-Based Copy", systemImage: "network.badge.shield.half.filled")
            .font(.headline)
          Text(
            "Uses chunk-based copying for network volumes (SMB/NAS). Enables cancellation, speed limiting, and better timeout handling. When disabled, uses NSFileCoordinator which may be more compatible but cannot be cancelled mid-file."
          )
          .font(.callout)
          ...
          Label("Network Timeout", systemImage: "clock.badge.exclamationmark")
            .font(.headline)
          Text(
            "Automatically abort network copies that stall for the configured time (default: 90 seconds). Prevents indefinite hangs on SMB issues."
          )
          .font(.callout)
          ...
          Label("Speed Limiting", systemImage: "speedometer")
            .font(.headline)
          Text(
            "Throttle network copy speed to prevent overwhelming SMB connections. Try 50-100 MB/s if experiencing disconnections."
          )
          .font(.callout)
          ...
          Label("Buffer Size", systemImage: "square.stack.3d.up")
            .font(.headline)
          Text(
            "Smaller buffers (1-2 MB) can be more stable on flaky connections. Larger buffers (8-16 MB) may be faster on stable networks."
          )
          .font(.callout)
        }
      }
      ...
      GroupBox("Common Issues") {
        VStack(alignment: .leading, spacing: 12) {
          TroubleshootingItem(
            ...
            solution:
              "Enable 'Stream-based copy' in Preferences → Performance. Try setting a speed limit (50-100 MB/s) and smaller buffer size (1-2 MB). The timeout is configurable (default 90 seconds)."
          )

          TroubleshootingItem(
            issue: "Network copies cannot be cancelled",
            solution:
              "Enable 'Stream-based copy' in Preferences → Performance. When disabled, the traditional NSFileCoordinator method cannot be cancelled mid-file."
          )
          ...
          TroubleshootingItem(
            ...
            solution:
              "Check Activity Monitor. Other apps may be using disk heavily. Close unnecessary applications."
          )

          TroubleshootingItem(
            issue: "Files not copying",
            solution:
              "Check file type filter settings. Ensure files aren't being skipped by duplicate detection."
          )

          TroubleshootingItem(
            issue: "Backup won't start",
            solution:
              "Verify source folder exists and destinations are mounted. Check for disk space."
          )
        }
      }
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        FAQItem(
          ...
          answer:
            "ImageIntact only backs up photography-related files (images, videos, RAW files, and sidecar files). System files, application files, document files, and cache files are automatically excluded. Additionally, symbolic links (shortcuts/aliases) are skipped for security reasons."
        )

        FAQItem(
          question: "What are symbolic links and why are they skipped?",
          answer:
            "Symbolic links (also called symlinks or aliases on Mac) are special files that act as shortcuts pointing to other files or folders. For security and data integrity, ImageIntact backs up only the actual files, not these shortcuts. This prevents potential security issues and ensures you're backing up real data, not just pointers to data."
        )
        ...
        FAQItem(
          ...
          answer:
            "macOS SMB can be unstable with high-speed transfers. Go to Preferences → Performance → Network Performance and: 1) Enable 'Stream-based copy' for cancellable operations, 2) Set a speed limit (try 50-100 MB/s), 3) Use smaller buffer size (1-2 MB), 4) Adjust timeout (default 90 seconds). These settings only affect network volumes, not local drives."
        )

        FAQItem(
          question:
            "What's the difference between Stream-based and NSFileCoordinator copy methods?",
          answer:
            "Stream-based copy (recommended for network drives) copies files in chunks, allowing cancellation, speed limiting, and timeout handling. NSFileCoordinator uses Apple's file coordination system - it may be more compatible with some servers but cannot be cancelled once a file copy starts and may hang indefinitely on network issues."
        )

        FAQItem(
          question: "Can I backup to the same drive multiple times?",
          answer:
            "Yes! Each destination must be a different folder. This is useful for creating multiple organized copies or different organizational structures of the same photos."
        )

        FAQItem(
          question: "What happens if a backup is interrupted?",
          answer:
            "ImageIntact is designed to handle interruptions safely. On the next run, it will skip files that were already successfully copied (verified by checksum) and continue with the remaining files. Your data is always safe - files are never deleted or overwritten without quarantine."
        )

        FAQItem(
          question: "Why are some file types like .cosessiondb or .lrdata not being copied?",
          answer:
            "These are package files (folders that appear as single files) containing cache data and previews. ImageIntact intelligently skips cache folders inside these packages while still backing up the important catalog and session files."
        )

        FAQItem(
          question:
            "What's the difference between 'Skip Exact Duplicates' and 'Skip Renamed Duplicates'?",
          answer:
            "'Skip Exact Duplicates' skips files with identical names and content. 'Skip Renamed Duplicates' also skips files with the same content but different names (useful when you've renamed files but they're the same photo)."
        )

        FAQItem(
          question: "How does the checksum verification work?",
          answer:
            "Every file is verified using SHA-256 checksums - a cryptographic method that creates a unique fingerprint for each file. After copying, ImageIntact verifies the destination file matches the source exactly. This ensures perfect, bit-for-bit copies."
        )

        FAQItem(
          question: "Can I use ImageIntact with memory cards?",
          answer:
            "Yes, but ImageIntact will warn you if you try to select a memory card as a destination (writing to cards can be slow and reduce their lifespan). Memory cards are perfectly fine as source locations."
        )

        FAQItem(
          question: "Why does ImageIntact need Full Disk Access?",
          answer:
            "Full Disk Access is required to read from and write to external drives and protected folders. Without it, macOS prevents access to many locations including external drives. Your privacy is protected - ImageIntact never sends data online."
        )
      }
      .padding()
    }
