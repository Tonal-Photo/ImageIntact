    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if backupManager.sourceURL != nil && !backupManager.isScanning {
                    Toggle(isOn: Binding(
                        get: { backupManager.includeSubdirectories },
                        set: { newValue in
                            backupManager.includeSubdirectories = newValue
                        }
                    )) {
                        HStack(spacing: 4) {
                ...
                if backupManager.sourceURL != nil {
                    HStack(alignment: .top) {
                        ...
                        let isDuplicatePreset = BackupPresetManager.shared
                            .currentConfigurationMatchesExistingPreset(backupManager: backupManager)
                        ...
                        if hasValidConfiguration && !backupManager.isScanning {
                            Button(action: {
                                ...
                                NotificationCenter.default.post(
                                    name: NSNotification.Name("ShowCreatePreset"), object: nil
                                )
                            }) {
                                Text("Save as Preset")
                                    .font(.caption)
                            ...
                            .help(
                                isDuplicatePreset
                                    ? "This configuration already exists as a preset"
