...
extension EventLogger {
    ...
    func batchInsertEvents(_ events: [PendingEvent]) {
        ...
        backgroundContext.perform { [weak self] in
            ...
            guard let session = try? self.backgroundContext.fetch(sessionRequest).first else {
                ApplicationLogger.shared.warning("Session not found for batch event logging", category: .database)
                return
            }
            ...
            do {
                ...
                ApplicationLogger.shared.debug("Batch saved \(events.count) events", category: .database)
            } catch {
                ApplicationLogger.shared.error("Failed to batch save events: \(error)", category: .database)
            }
        }
    }
    ...
    func batchInsertEventsOptimized(_ events: [PendingEvent]) {
        ...
        backgroundContext.perform { [weak self] in
            ...
            guard let session = try? self.backgroundContext.fetch(sessionRequest).first else {
                ApplicationLogger.shared.warning("Session not found for batch insert", category: .database)
                return
            }
            ...
            do {
                ...
                {
                    ApplicationLogger.shared.debug("Batch inserted \(objectIDs.count) events using NSBatchInsertRequest", category: .database)
                }
            } catch {
                ApplicationLogger.shared.error("Failed to batch insert events: \(error)", category: .database)
                ...
                do {
                    try self.backgroundContext.save()
                    ApplicationLogger.shared.debug("Fallback batch saved \(events.count) events", category: .database)
                } catch {
                    ApplicationLogger.shared.error("Failed to fallback save events: \(error)", category: .database)
                }
            }
        }
    }
