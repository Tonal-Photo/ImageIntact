...
class ProgressTracker: ObservableObject, ProgressTrackerProtocol {
  ...
  func updateETA() {
    ...
    if recentSpeedSamples.count >= 5 {
      averageSpeed =
        recentSpeedSamples.suffix(10).reduce(0, +) / Double(min(10, recentSpeedSamples.count))
    } else {
    ...
    #if DEBUG
      print(
        "ETA Debug: totalBytesToCopy=\(totalBytesToCopy), totalBytesCopied=\(totalBytesCopied), remainingBytes=\(remainingBytes), averageSpeed=\(averageSpeed) MB/s, copySpeed=\(copySpeed) MB/s"
      )
    #endif
  }
  ...
  func updateScanProgress(_: String) {
