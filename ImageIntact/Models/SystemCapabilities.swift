...
class SystemCapabilities {
    ...
    private init() {
        ...
        container.loadPersistentStores { _, error in
            if let error = error {
                ApplicationLogger.shared.error("Failed to load SystemInfo store: \(error)", category: .hardware)
            }
        }
    ...
    func detectSystem() {
        ...
        ApplicationLogger.shared.debug("System Detection Complete:", category: .hardware)
        ApplicationLogger.shared.debug("  Processor: \(processorName)", category: .hardware)
        ApplicationLogger.shared.debug("  Type: \(processorType.rawValue)", category: .hardware)
        ApplicationLogger.shared.debug("  Architecture: \(arch)", category: .hardware)
        ApplicationLogger.shared.debug("  Total Cores: \(cpuCores)", category: .hardware)
        if perfCores > 0 {
            ApplicationLogger.shared.debug("  Performance Cores: \(perfCores)", category: .hardware)
            ApplicationLogger.shared.debug("  Efficiency Cores: \(effCores)", category: .hardware)
        }
        ApplicationLogger.shared.debug("  RAM: \(formatBytes(totalRAM))", category: .hardware)
        ApplicationLogger.shared.debug("  Neural Engine: \(processorType.hasNeuralEngine ? "Yes" : "No")", category: .hardware)
    }
    ...
    func saveToDatabase() {
        ...
        if shouldCreateNew {
            ...
            ApplicationLogger.shared.debug("Creating new system info entry: \(changeReason)", category: .hardware)
        } else {
            // Just update the timestamp on existing entry
            if let recent = mostRecent {
                recent.detectedAt = Date()
                ApplicationLogger.shared.debug("System unchanged, updating timestamp only", category: .hardware)
            }
        }

        do {
            try context.save()
            ApplicationLogger.shared.debug("System info saved to database", category: .hardware)
        } catch {
            ApplicationLogger.shared.error("Failed to save system info: \(error)", category: .hardware)
        }
    }
    ...
    func getSystemHistory(limit: Int = 100) -> [SystemInfoEntity] {
        ...
        do {
            ...
            ApplicationLogger.shared.error("Failed to fetch system history: \(error)", category: .hardware)
            return []
        }
    }
    ...
    func getUniqueSystems() -> [(processor: String, count: Int, lastSeen: Date)] {
        ...
        do {
            ...
            ApplicationLogger.shared.error("Failed to fetch unique systems: \(error)", category: .hardware)
            return []
        }
    }
}
