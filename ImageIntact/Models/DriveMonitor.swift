...
class DriveMonitor: ObservableObject {
  ...
  private func setupDiskArbitration() {
    ...
    DARegisterDiskAppearedCallback(
      session, nil, driveAppearedCallback, Unmanaged.passUnretained(self).toOpaque())

    // Register for disk disappeared notifications
    DARegisterDiskDisappearedCallback(
      session, nil, driveDisappearedCallback, Unmanaged.passUnretained(self).toOpaque())

    // Register for disk description changed notifications (fixed parameter order)
    DARegisterDiskDescriptionChangedCallback(
      session, nil, nil, driveChangedCallback, Unmanaged.passUnretained(self).toOpaque())
  }
