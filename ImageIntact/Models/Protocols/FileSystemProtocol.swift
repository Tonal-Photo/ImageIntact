...
final class MockFileSystem: FileSystemProtocol, @unchecked Sendable {
    ...
    func createDirectory(at url: URL, withIntermediateDirectories: Bool) throws {
        if shouldFailCreate {
            throw NSError(domain: "MockFileSystem", code: 1, userInfo: [NSLocalizedDescriptionKey: "Mock create directory failed"])
        }
    ...
    func copyItem(at srcURL: URL, to dstURL: URL) throws {
        if shouldFailCopy {
            throw NSError(domain: "MockFileSystem", code: 2, userInfo: [NSLocalizedDescriptionKey: "Mock copy failed"])
        }
        
        guard files.contains(srcURL.path) else {
            throw NSError(domain: "MockFileSystem", code: 3, userInfo: [NSLocalizedDescriptionKey: "Source file not found"])
        }
    ...
    func moveItem(at srcURL: URL, to dstURL: URL) throws {
        if shouldFailMove {
            throw NSError(domain: "MockFileSystem", code: 4, userInfo: [NSLocalizedDescriptionKey: "Mock move failed"])
        }
    ...
    func contentsOfDirectory(at url: URL, includingPropertiesForKeys keys: [URLResourceKey]?) throws -> [URL] {
        guard directories.contains(url.path) else {
            throw NSError(domain: "MockFileSystem", code: 5, userInfo: [NSLocalizedDescriptionKey: "Directory not found"])
        }
    ...
    func attributesOfItem(atPath path: String) throws -> [FileAttributeKey: Any] {
        guard files.contains(path) || directories.contains(path) else {
            throw NSError(domain: "MockFileSystem", code: 6, userInfo: [NSLocalizedDescriptionKey: "Item not found"])
        }
        
        return fileAttributes[path] ?? [
            .size: fileContents[path]?.count ?? 0,
            .type: directories.contains(path) ? FileAttributeType.typeDirectory : FileAttributeType.typeRegular
        ]
    }
    
    func setAttributes(_ attributes: [FileAttributeKey: Any], ofItemAtPath path: String) throws {
        guard files.contains(path) || directories.contains(path) else {
            throw NSError(domain: "MockFileSystem", code: 7, userInfo: [NSLocalizedDescriptionKey: "Item not found"])
        }
