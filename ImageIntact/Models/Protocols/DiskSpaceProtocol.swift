...
protocol DiskSpaceProtocol {
  func checkDestinationSpace(destination: URL, requiredBytes: Int64, additionalBuffer: Int64)
  ...
  func evaluateSpaceChecks(_ results: [DiskSpaceChecker.SpaceCheckResult]) -> (
    canProceed: Bool, warnings: [String], errors: [String]
  )
}
...
final class RealDiskSpaceChecker: DiskSpaceProtocol {
  ...
  func checkDestinationSpace(
    ...
    return DiskSpaceChecker.checkDestinationSpace(
      destination: destination, requiredBytes: requiredBytes, additionalBuffer: additionalBuffer)
  }

  func checkAllDestinations(destinations: [URL], requiredBytes: Int64) -> [DiskSpaceChecker
    .SpaceCheckResult]
  {
    return DiskSpaceChecker.checkAllDestinations(
      destinations: destinations, requiredBytes: requiredBytes)
  }
...
final class MockDiskSpaceChecker: DiskSpaceProtocol {
  ...
  func checkDestinationSpace(
    ...
    let spaceInfo =
      mockSpaceInfo[destination]
      ?? DiskSpaceChecker.DiskSpaceInfo(
    ...
    let hasEnoughSpace =
      mockHasEnoughSpace[destination]
      ?? (spaceInfo.availableSpace >= requiredBytes + additionalBuffer)
    let warning = mockWarnings[destination] ?? nil
    let error = mockErrors[destination] ?? nil

    // Calculate if will have less than 10% free
    let spaceAfterBackup =
      spaceInfo.freeSpace > requiredBytes ? spaceInfo.freeSpace - requiredBytes : 0
    let percentFreeAfterBackup =
      spaceInfo.totalSpace > 0
      ? (Double(spaceAfterBackup) / Double(spaceInfo.totalSpace)) * 100 : 0.0
  ...
  func simulateInsufficientSpace(for url: URL, requiredBytes: Int64 = 100_000_000_000) {
    configureMockSpace(
      ...
      hasEnoughSpace: false,
      error:
        "Insufficient space: Need \(ByteCountFormatter.string(fromByteCount: requiredBytes, countStyle: .file)) but only 10 GB available"
    )
  }
}
