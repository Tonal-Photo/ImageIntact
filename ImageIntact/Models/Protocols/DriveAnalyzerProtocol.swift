...
final class RealDriveAnalyzer: DriveAnalyzerProtocol {
    ...
    func isNetworkVolume(at url: URL) -> Bool {
        ...
        do {
            ...
            ApplicationLogger.shared.warning("Error checking if volume is network: \(error)", category: .hardware)
        }
    ...
    func getVolumeInfo(for url: URL) -> (total: Int64, available: Int64)? {
        do {
            ...
            ApplicationLogger.shared.warning("Error getting volume info: \(error)", category: .hardware)
            return nil
        }
    }
}
