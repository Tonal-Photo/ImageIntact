...
final class MockHasher: HashingProtocol {
  ...
  func sha256(for url: URL, shouldCancel: @escaping () -> Bool) async throws -> String {
    ...
    if shouldCancel() {
      throw NSError(
        domain: "MockHasher", code: 1, userInfo: [NSLocalizedDescriptionKey: "Operation cancelled"])
    }

    if shouldFail {
      throw NSError(
        domain: "MockHasher", code: 2, userInfo: [NSLocalizedDescriptionKey: "Mock hash failed"])
    }
