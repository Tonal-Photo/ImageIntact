...
class ApplicationLogger {
    ...
    private init() {
        ...
        // Observe verbose logging toggle
        DebugSettings.shared.$verboseLogging
            .sink { [weak self] isVerbose in
                if isVerbose {
                    self?.minimumLogLevel = .debug
                } else {
                    // Restore to persisted preference
                    let savedLevel = PreferencesManager.shared.minimumLogLevel
                    self?.minimumLogLevel = LogLevel(rawValue: Int16(savedLevel)) ?? .info
                }
            }
    ...
    // Convenience methods
    func debug(
        _ message: String, category: LogCategory = .app,
        file: String = #file, function: String = #function, line: Int = #line
    ) {
    ...
    func logError(
        _ error: Error, errorContext: String? = nil, recovered: Bool = true,
        file: String = #file, function: String = #function, line: Int = #line
    ) {
        let filename = URL(fileURLWithPath: file).lastPathComponent
        ...
        log(
            .error, .error, "Error: \(error.localizedDescription) - Context: \(errorContext ?? "none")",
    ...
    func logPerformance(
        operation: String, startTime: Date, endTime: Date = Date(),
    ...
    func fetchLogs(
        since date: Date? = nil, level: LogLevel? = nil,
        category: LogCategory? = nil, limit: Int = 1000
    ...
    private func exportAsCSV(_ logs: [ApplicationLogEntity]) -> Data? {
        ...
        for log in logs {
            ...
            csv +=
                "\(timestamp),\(level),\(log.category ?? ""),\"\(message)\",\(log.file ?? ""),\(log.function ?? ""),\(log.line)\n"
        }
...
func logDebug(
    ...
    ApplicationLogger.shared.debug(
        message, category: category, file: file, function: function, line: line
    )
}
...
func logCritical(
    ...
    ApplicationLogger.shared.critical(
        message, category: category, file: file, function: function, line: line
    )
}
