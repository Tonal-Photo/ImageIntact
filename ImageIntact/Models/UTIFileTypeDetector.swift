...
class UTIFileTypeDetector {
    ...
    func isSupportedFile(_ url: URL) -> Bool {
        ...
        if #available(macOS 11.0, *) {
            if let type = getUTType(for: url), isSupportedUTType(type) {
                return true
            }
        } else {
            // For older macOS, try legacy UTI detection
            if let type = getLegacyUTI(for: url), isSupportedLegacyUTI(type) {
                return true
            }
        }

        // Fallback to extension-based detection (for files with unknown UTIs like AAE)
        let ext = url.pathExtension.lowercased()
        return ImageFileType.from(fileExtension: ext) != nil
    }
    ...
    private func getUTType(for url: URL) -> UTType? {
        do {
            ...
            ApplicationLogger.shared.warning("Could not get UTI for \(url.lastPathComponent): \(error)", category: .fileSystem)
        }
