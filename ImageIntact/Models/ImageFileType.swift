...
class ImageFileScanner {
    ...
    func scan(
        ...
        let task = Task<ScanResult, Error> {
            ...
            guard
                ...
                throw NSError(
                    domain: "ImageFileScanner", code: 1,
            ...
            let photoPackageExtensions = Set([
                "cosessiondb", "lrdata", "photoslibrary", "aplibrary", "photolibrary",
            ])
    ...
    func scanWithSize(
        directory: URL,
        progress: @escaping (ScanProgress) -> Void
    ) async throws -> DetailedScanResult {
        // Cancel any existing scan
        currentTask?.cancel()

        ApplicationLogger.shared.debug("Starting scan of: \(directory.path)", category: .fileSystem)
        ...
        while let element = enumerator.nextObject() {
            ...
            let depth = url.pathComponents.count - directory.pathComponents.count
            if depth > maxDepth {
                ApplicationLogger.shared.debug("Skipping deep path (depth \(depth)): \(url.path)", category: .fileSystem)
                enumerator.skipDescendants()
                continue
            }
            ...
            if resourceValues.isSymbolicLink == true {
                ...
                do {
                    let resolvedURL = try URL(
                        fileURLWithPath: FileManager.default.destinationOfSymbolicLink(atPath: url.path))
                    let realPath = resolvedURL.path

                    // Check for circular references
                    if visitedPaths.contains(realPath) {
                        ApplicationLogger.shared.debug("Skipping circular symlink: \(url.lastPathComponent) -> \(realPath)", category: .fileSystem)
                        enumerator.skipDescendants()
                        continue
                    }
                    ...
                    if !realPath.hasPrefix(directory.path) {
                        ApplicationLogger.shared.debug("Skipping external symlink: \(url.lastPathComponent) -> \(realPath)", category: .fileSystem)
                        enumerator.skipDescendants()
                        continue
                    }
                } catch {
                    ApplicationLogger.shared.debug("Cannot resolve symlink: \(url.lastPathComponent)", category: .fileSystem)
                    enumerator.skipDescendants()
                    continue
                }
            }
            ...
            if resourceValues.isPackage == true {
                ...
                if !photoPackageExtensions.contains(ext) {
                    ...
                    if skippedPackages <= 5 {
                        ApplicationLogger.shared.debug("Skipping package: \(url.lastPathComponent)", category: .fileSystem)
                    }
                    enumerator.skipDescendants()
                    continue
                }
                // If it's a photo package, we'll scan inside it
                ApplicationLogger.shared.debug("Scanning photo package: \(url.lastPathComponent)", category: .fileSystem)
            }
            ...
            if let fileType = ImageFileType.from(fileExtension: ext) {
                ...
                if fileType.isRaw {
                    ...
                    if rawFileCount <= 5 || rawFileCount % 100 == 0 {
                        ApplicationLogger.shared.debug("RAW #\(rawFileCount): \(url.lastPathComponent) (\(fileType.rawValue))", category: .fileSystem)
                    }
                }
                ...
                skippedCount += 1
                if skippedCount <= 10 {
                    ApplicationLogger.shared.debug("Skipped unsupported: .\(ext) - \(url.lastPathComponent)", category: .fileSystem)
                }
            }
        ...
        ApplicationLogger.shared.debug("Scan complete:", category: .fileSystem)
        ApplicationLogger.shared.debug("Total files scanned: \(scannedCount)", category: .fileSystem)
        ApplicationLogger.shared.debug("RAW files found: \(rawFileCount)", category: .fileSystem)
        ApplicationLogger.shared.debug("Files skipped (unsupported): \(skippedCount)", category: .fileSystem)
        ApplicationLogger.shared.debug("Packages skipped: \(skippedPackages)", category: .fileSystem)
        ApplicationLogger.shared.debug("Symlinks resolved: \(resolvedSymlinks)", category: .fileSystem)
        ApplicationLogger.shared.debug("Total size: \(totalBytes / (1024 * 1024 * 1024)) GB", category: .fileSystem)

        // Log breakdown by type
        for (type, count) in fileTypes.sorted(by: { $0.value > $1.value }) {
            ApplicationLogger.shared.debug("\(type.rawValue): \(count)", category: .fileSystem)
        }
    ...
    static func formatScanResults(_ results: ScanResult, groupRaw _: Bool = false) -> String {
        if results.isEmpty {
