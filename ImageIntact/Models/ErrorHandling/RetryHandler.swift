        ...
        for attempt in 1 ... strategy.maxAttempts {
            do {
                ...
                if attempt > 1 {
                    ...
                    ApplicationLogger.shared.debug("Retry successful for \(operation) after \(attempt) attempts", category: .backup)
                }
                ...
                if shouldRetry {
                    ...
                    ApplicationLogger.shared.warning(
                        "\(operation) failed (attempt \(attempt)/\(strategy.maxAttempts)): \(error.localizedDescription)",
                        category: .backup
                    )
                    ApplicationLogger.shared.debug("   Retrying in \(String(format: "%.1f", delay)) seconds...", category: .backup)
                    ...
                    if attempt > 1 {
                        ...
                        ApplicationLogger.shared.error("Retry failed for \(operation) after \(attempt) attempts", category: .backup)
                    }
                    throw error
                }
            }
