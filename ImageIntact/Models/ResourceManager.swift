...
actor ResourceManager {
    ...
    func checkResourceLimits() -> Bool {
        ...
        if taskCount > maxConcurrentOperations {
            ApplicationLogger.shared.warning("Resource warning: \(taskCount) active tasks (limit: \(maxConcurrentOperations))", category: .app)
            return false
        }

        if handleCount > 100 {
            ApplicationLogger.shared.warning("Resource warning: \(handleCount) open file handles", category: .app)
            return false
        }
    ...
    func cleanup() {
        ApplicationLogger.shared.debug("Cleaning up resources...", category: .app)
        ...
        ApplicationLogger.shared.debug("Resource cleanup complete", category: .app)
    }
