...
class BackupStatistics: ObservableObject {
  ...
  func generateSummary() -> String {
    ...
    if !fileTypeStats.isEmpty {
      summary += "\nBy File Type:\n"
      for (type, stats) in fileTypeStats.sorted(by: {
        $0.value.filesProcessed > $1.value.filesProcessed
      }) {
        if stats.filesProcessed > 0 {
          summary +=
            "  \(type.rawValue): \(stats.successCount) files, \(formatBytes(stats.totalBytes))\n"
        }
      }
    }
