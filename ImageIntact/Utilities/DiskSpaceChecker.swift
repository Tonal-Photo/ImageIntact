...
class DiskSpaceChecker {
  ...
  static func checkDestinationSpace(
    ...
    guard let spaceInfo = getDiskSpaceInfo(for: destination) else {
      return SpaceCheckResult(
        ...
        spaceInfo: DiskSpaceInfo(
          totalSpace: 0, freeSpace: 0, availableSpace: 0, percentFree: 0, percentAvailable: 0),
    ...
    if !hasEnoughSpace {
      error = String(
        format: "Insufficient space: Need %@ but only %@ available",
      ...
      warning = String(
        format: "Low disk space warning: After backup, only %.1f%% will remain free",
        percentFreeAfterBackup)
    }
      ...
      if statfs(url.path, &stat) == 0 {
        ...
        if totalSpace > 0 && availableSpace >= 0 && freeSpace >= 0 {
          ...
          let percentAvailable =
            totalSpace > 0 ? (Double(availableSpace) / Double(totalSpace)) * 100 : 0
        ...
        logInfo(
          "Network volume \(url.path) reported unreliable space values, trying alternate methods")
      }
    }
  ...
  static func evaluateSpaceChecks(_ results: [SpaceCheckResult]) -> (
