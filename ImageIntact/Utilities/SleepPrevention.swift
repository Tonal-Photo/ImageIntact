...
class SleepPrevention {
  ...
  func startPreventingSleep(
    reason: String = "ImageIntact Backup in Progress", timeout: TimeInterval? = nil
  ) -> Bool {
    lock.lock()
    defer { lock.unlock() }
    ...
    if !isPreventingSleep {
      ...
      if result == kIOReturnSuccess {
        ...
        ApplicationLogger.shared.error(
          "Failed to prevent sleep: IOKit error \(result)", category: .performance)
        return false
      }
    }
  ...
  private func handleTimeout() {
    ...
    ApplicationLogger.shared.warning(
      "Sleep prevention timeout reached after \(maxDuration/3600) hours", category: .performance)
    stopPreventingSleep()
  }
  ...
  func stopPreventingSleep() {
    ...
    if result == kIOReturnSuccess {
      ...
      ApplicationLogger.shared.error(
        "Failed to release sleep prevention: IOKit error \(result)", category: .performance)
    }
  }
