...
class DriveIdentityManager: ObservableObject {
  ...
  func findOrCreateDriveIdentity(for driveInfo: DriveAnalyzer.DriveInfo) -> DriveIdentity? {
    ...
    guard container.persistentStoreCoordinator.persistentStores.count > 0 else {
      ApplicationLogger.shared.error(
        "Core Data not available for drive identity", category: .database)
      return nil
    }
    ...
    if driveInfo.driveType == .cameraCard || driveInfo.driveType == .cardReader
      || driveInfo.driveType == .inCamera
    {
  ...
  func updateDriveCustomization(
    _ drive: DriveIdentity, name: String?, emoji: String?, location: String?, notes: String?
  ) {
  ...
  private func handleDriveConnected(_ driveInfo: DriveAnalyzer.DriveInfo) {
    guard let identity = findOrCreateDriveIdentity(for: driveInfo) else {
      ApplicationLogger.shared.info(
        "Drive connected (no identity): \(driveInfo.deviceName)", category: .app)
      return
    }
    ...
    ApplicationLogger.shared.info(
      "Drive connected: \(identity.userLabel ?? driveInfo.deviceName)", category: .app)
  }
  ...
  private func saveContext() {
    guard container.persistentStoreCoordinator.persistentStores.count > 0 else {
      ApplicationLogger.shared.error(
        "Cannot save: No persistent stores available", category: .database)
      return
    }
  ...
  func isConnected(_ drive: DriveIdentity) -> Bool {
    return connectedDrives.contains { driveInfo in
      driveInfo.volumeUUID == drive.volumeUUID || driveInfo.hardwareSerial == drive.hardwareSerial
    }
  }
