...
class SecurityEnhancementTests: XCTestCase {
    ...
    func testSymbolicLinkDetectionAndSkipping() async throws {
        ...
        // Should throw an error for symbolic links (security protection)
        do {
            try await fileOps.copyItem(at: symlinkPath, to: dest)
            XCTFail("Should have thrown an error for symbolic link")
        } catch {
            // Expected: symlinks are rejected
            let nsError = error as NSError
            XCTAssertEqual(nsError.code, 110, "Error code should indicate symlink rejection")
        }

        // Destination should not exist (symlink was rejected)
    ...
    func testSleepPreventionTimeout() {
        ...
        // Ensure clean state - stop any existing prevention from other tests
        sleepPrevention.stopPreventingSleep()

        // Start prevention and verify it was enabled
        let started = sleepPrevention.startPreventingSleep(reason: "Test", timeout: 60)

        // Only assert if start succeeded (might fail if PreferencesManager disabled it)
        if started {
            XCTAssertTrue(sleepPrevention.isPreventing, "Should be preventing sleep after start")

            // Stop prevention
            sleepPrevention.stopPreventingSleep()
            XCTAssertFalse(sleepPrevention.isPreventing, "Should not be preventing sleep after stop")
        } else {
            // If start failed, isPreventing should still be false
            XCTAssertFalse(sleepPrevention.isPreventing,
                           "Sleep prevention not enabled (preference may be disabled)")
        }
    }
