...
final class NativeChecksumTests: XCTestCase {
    ...
    func testSHA256ChecksumForSmallFile() throws {
        ...
        // Calculate checksum
        let checksum = try ChecksumService.sha256(for: testFile, shouldCancel: { false })
        ...
        // Verify consistency
        let checksum2 = try ChecksumService.sha256(for: testFile, shouldCancel: { false })
    ...
    func testSHA256ChecksumStreamingThreshold() throws {
        ...
        let smallChecksum = try ChecksumService.sha256(for: smallFile, shouldCancel: { false })
        let largeChecksum = try ChecksumService.sha256(for: largeFile, shouldCancel: { false })
        ...
        // Verify consistency
        let smallChecksum2 = try ChecksumService.sha256(for: smallFile, shouldCancel: { false })
        let largeChecksum2 = try ChecksumService.sha256(for: largeFile, shouldCancel: { false })
    ...
    func testChecksumMatchingForIdenticalFiles() throws {
        ...
        let checksum1 = try ChecksumService.sha256(for: file1, shouldCancel: { false })
        let checksum2 = try ChecksumService.sha256(for: file2, shouldCancel: { false })
    ...
    func testChecksumPerformanceSmallFiles() throws {
        ...
        measure {
            for file in files {
                _ = try? ChecksumService.sha256(for: file, shouldCancel: { false })
            }
        }
    }
    ...
    func testChecksumForFileWithSpecialCharacters() throws {
        ...
        XCTAssertNoThrow(try ChecksumService.sha256(for: file, shouldCancel: { false }))
    }
