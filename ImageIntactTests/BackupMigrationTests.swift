...
class BackupMigrationTests: XCTestCase {
    ...
    func testDetectsNoMigrationNeeded_WhenDestinationEmpty() async throws {
        ...
        manifest = try [
            FileManifestEntry(
                ...
                checksum: calculateChecksum(for: sourceFile),
                size: 4
            ),
        ]
    ...
    func testDetectsMigrationNeeded_WhenMatchingFilesInRoot() async throws {
        ...
        let checksum = try ChecksumService.sha256(for: sourceFile, shouldCancel: { false })
        manifest = [
            FileManifestEntry(
                relativePath: "photo.jpg",
                sourceURL: sourceFile,
    ...
    func testMigrationVerifiesChecksumAfterMove() async throws {
        ...
        let movedChecksum = try ChecksumService.sha256(for: movedFile, shouldCancel: { false })
